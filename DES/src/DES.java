...
//=========================================================Permutations=======================================================================//
    ...
    public static byte[] InitialPermutation(byte[] text){ //initial permutation func 1001 1001 1111 0011 -> 0000 0000 0000 0001
        ...
        for (int i = 0; i < IP.length; i++) { //goes through IP table and initializing the new output array with correct index
    ...
    public static byte[] FinalPermutation(byte[] text){
        ...
        for (int i = 0; i < IPMinusOne.length; i++) { //goes through IP-1 table and initializing the new output array with correct index
            int bitInx = IPMinusOne[i] - 1; // represents the bit index of IP-1 table
            int byteInx = bitInx / 8; //calculates the byte index in array by simply dividing by 8 because we know all bytes are size of 8
    ...
    public static byte[] PCMinusOnePermutation(byte[] text){
        ...
        for (int i = 0; i < PCMinusOne.length; i++) { //goes through PCMinusOne table and initializing the new output array with correct index
    ...
    public static byte[] PCMinusTwoPermutation(byte[] text){
        ...
        for (int i = 0; i < PCMinusTwo.length; i++) { //goes through PCMinusTwo table and initializing the new output array with correct index
    ...
    public static byte[] expansionPermutation(byte[] text){
        byte[] output = new byte[6]; //allocating of output array

        for (int i = 0; i < E.length; i++) { //goes through E table and initializing the new output array with correct index
    ...
    public static byte[] pPermutation(byte[] text){
        ...
        for (int i = 0; i < P.length; i++) { //goes through P table and initializing the new output array with correct index
...
//============================================================================================================================================//
    ...
    public static byte[][] KeySchedule(byte[] key, String mode){
        ...
        //split the key byte array to form two 28 bit halves represented in int
        int left = 0;
        int right = 0;
        ...
        for(int i = 0; i < keys.length; i++) {
            ...
            //does OR to add the right's MSB bit to the right side
            ...
            // merging the left and right ints to byte array
            ByteBuffer buffer = ByteBuffer.allocate(7); //allocation of 7 bytes (56 bits)
            buffer.put((byte) (left >>> 20)); //inserting first byte of left (most left 8 bits)
            buffer.put((byte) (left >>> 12)); //inserting next 8bits of left (remember that cast to byte takes the least significant bits)
            ...
            currentKey = PCMinusTwoPermutation(currentKey); //sending current key to second permutation in each round
...
//==============================================================================================================//
    ...
    public static byte[] F(byte[] RightText, byte[] key){
        ...
        byte[] NewRight = expansionPermutation(RightText); //saving the new permuted RightText in new variable
    ...
    public static byte[] DESFunction(byte[] text, byte[] key, String mode){ // mode can be only "encrypt" or "decrypt"
        ...
        if(!Objects.equals(mode, "encrypt") && !Objects.equals(mode, "decrypt"))//checks if mode type is valid
